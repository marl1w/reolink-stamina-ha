...
PORT     ?= 8123
SEED     ?= 1
# The preview invents every state by default, so there is nothing to choose. DAYS gives every
# camera the same history instead, which is occasionally useful and never necessary.
DAYS     ?=
JOURNAL  ?=
ARGS     ?=
...
help: ## List the targets
	...
	@printf '\n$(DIM)Variables: PORT=%s SEED=%s DAYS= JOURNAL=<path> ARGS=<extra>$(OFF)\n\n' \
	  "$(PORT)" "$(SEED)"
...
preview: venv-check ## Serve the real panel with sample data in a browser
	@$(PY) scripts/preview.py --port $(PORT) --seed $(SEED) $(if $(DAYS),--days $(DAYS)) $(ARGS)
