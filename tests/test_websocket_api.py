...
async def test_detections_range_returns_absolute_times_per_camera(
    hass: HomeAssistant, hass_ws_client: WebSocketGenerator, setup_stamina
) -> None:
    """What the rows need to say "Person (2)".

    Offsets are meaningless across a whole range, so only absolute times travel — the panel
    matches them against the rows it already holds.
    """
    runs = [
        {"at": "2026-08-03T09:00:00+00:00", "kind": "person", "offset": 1.0, "until": None},
        {"at": "2026-08-03T09:04:00+00:00", "kind": "vehicle", "offset": 240.0, "until": None},
    ]

    with patch(
        "custom_components.reolink_stamina.websocket_api.async_detections_in_window",
        return_value=runs,
    ):
        client = await hass_ws_client(hass)
        await client.send_json_auto_id(
            {
                "type": f"{DOMAIN}/detections_range",
                "targets": [{"entry_id": setup_stamina.reolink.entry_id, "channel": 0}],
                "start_date": TODAY.isoformat(),
                "end_date": TODAY.isoformat(),
            }
    ...
    assert cameras[0]["key"] == f"{setup_stamina.reolink.entry_id}|0"
    assert cameras[0]["detections"] == [
        {"kind": "person", "at": "2026-08-03T09:00:00+00:00", "until": None},
        {"kind": "vehicle", "at": "2026-08-03T09:04:00+00:00", "until": None},
    ]


async def test_detections_range_asks_the_recorder_once_per_camera(
    hass: HomeAssistant, hass_ws_client: WebSocketGenerator, setup_stamina
) -> None:
    """A five-minute segmentation means ~300 rows a day; one query covers all of them."""
    with patch(
        "custom_components.reolink_stamina.websocket_api.async_detections_in_window",
        return_value=[],
    ) as query:
    ...
    # Whole days, so a detection anywhere in the range is covered.
    _hass, _entry, _channel, start, end = query.call_args[0]
    assert (start.hour, start.minute) == (0, 0)
    assert (end - start) == dt.timedelta(days=3)


async def test_detections_range_survives_a_recorder_that_cannot_answer(
    hass: HomeAssistant, hass_ws_client: WebSocketGenerator, setup_stamina
) -> None:
    """No counts is a fine outcome; a broken list is not."""
    client = await hass_ws_client(hass)
    await client.send_json_auto_id(
        {
            "type": f"{DOMAIN}/detections_range",
    ...
    assert response["success"]
    assert response["result"]["cameras"][0]["detections"] == []
